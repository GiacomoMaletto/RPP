...
then the \textbf{composition} $g \big( f_1(\vec{x}), \dots, f_m(\vec{x}) \big)$ belongs to $\PRF$.
...
\begin{align*}
  h(\vec{x}, 0)   = & f(\vec{x}) \\
  h(\vec{x}, y+1) = & g \big( \vec{x}, y, h(\vec{x}, y) \big)
\end{align*}
belongs to $\PRF$.
\end{itemize}
\end{definition}
...
if we've defined $z=h(\vec{x},y)$ then we can define $h(\vec{x},y+1)$ as $g(\vec{x},y,z)$,
which is something that only depends on the parameter $\vec{x}$, the iterator $y$ and the previously calculated value $z$.
...
Then, there exists $a \in \NN$ and $f : \ZZ^{1+k+a} \to \ZZ^{1+k+a}$ in $\RPP$ such that for all $z \in \ZZ$ and $x_1, \dots, x_k \in \NN$,
\[\begin{NiceMatrix}[nullify-dots]
  z      & \bloch{5-1}{f} & z + F(x_1, \dots, x_k) \\
  x_1    &                & x_1                    \\
  \Vdots &                & \Vdots                 \\
  x_k    &                & x_k                    \\
  0^a    &                & 0^a                    \\
\end{NiceMatrix}\]
where $0^a = (0, \dots, 0)$ is a tuple of length $a$.
\end{theorem}
