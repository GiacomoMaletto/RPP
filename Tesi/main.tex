...
theorem inv_co_l (f : RPP) (l : list ℤ) :
  ‹f ;; f⁻¹› l = l
\end{lstlisting}
We also need to prove \lstinline{inv_co_r} about the right inverse \lstinline{‹f⁻¹ ;; f› l = l},
...
Another one is $\rppmkpair$, which we will also denote with quotation marks $\gl \cdot, \cdot \gr$:
...
often we have something like \lstinline{‹inc› (n :: x ::l)} with \lstinline{l : list ℤ}.
...
more famous class of function: the \textit{Primitive Recursive Functions}.
...
and annoying to write down precisely, so for the moment no energies have been spent trying to formalize it in a proof assistant.

\paragraph{$\PRF$-completeness in Lean}

As usual, there's more than one way that our result can be proven.
...
building on top of what has been already done is both convenient and effective.

Truth to be told, there are at least two notions of $\PRF$ in Mathlib.
...
a function \lstinline{vector ℕ n → ℕ} and returns a proposition \lstinline{Prop} - in short, it's an \textbf{inductive proposition}.
...
but a bad one if instead we want to show a result about all $\PRF$.
...
investigating further, we discover that the function \lstinline{unpair} is the $\rppunpair$ function introduced in the previous section!
...
then $H$ is given by
\[ H(x) = [F, G]\big( (x)_1, (x)_2 \big) \]
where we define $[F,G] : \NN^2 \to \NN$ as
\begin{align*}
  [F, G](z,0) \ = \ & F(z) \\
  [F, G](z, n+1) \ = \ & G \big( \gl z, \gl n, \prPrec[F,G](z,n) \gr \gr \big)
\end{align*}

Let' assume that $f, g \in \RPP$ encode respectively $F, G \in PRF$, i.e.
$f(z, n, \boldsymbol{0}) = (z+F(n), n, \boldsymbol{0})$ and $g(z, n, \boldsymbol{0}) = (z+G(n), n, \boldsymbol{0})$.
Then, we simulate a single iteration of $[F, G]$ by $\rppprecstep[g] \in RPP$ given by
...
What's interesting about this function is that we use the functions \lstinline{mkpair} and \lstinline{unpair} for
two completely different purposes:
\begin{itemize}
\item The first two calls to $\rppmkpair$ are just there to create the value $\gl z, \gl n, \prPrec[F,G](z,n) \gr \gr$,
which is needed in order to calculate $[F,G](z,n+1)$, while the two $\rppunpair$ unwind the previous pairings.
\item The last mkpair is instead our answer to the problem of how to store information in excess.
Because we have a problem: there's a point in which both $[F,G](z,n+1)$ and $[F,G](z,n)$ are present at the same time.
We're interested in going forward with the iteration, so we don't need $[F,G](z,n)$ anymore;
but we can't just replace it with $0$ because that's not a reverisible action, and we also can't store each iteration $[F,G](z,0), [F,G](z,1), \dots$ in
...
by calling $\rppmkpair$ with arguments $s$, $[F,G](n,z)$ we obtain in the first one a new junk value $s'$, but more importantly
...
\noindent\begin{minipage}{.5\linewidth}
  ...
  \begin{minipage}{.5\linewidth}
  \[\begin{NiceMatrix}
    z                 & \bloch{7-1}{\rppprecfwd[f,g]} & H \big( (n)_1, (n)_2 \big)  \\
    n                 &                               & z                                      \\
    0                 &                               & (n)_2                                  \\
    0                 &                               & s                                      \\
    0                 &                               & (n)_1                                  \\
  ...
  \noindent\begin{minipage}{.5\linewidth}
    ...
    \begin{minipage}{.5\linewidth}
    \[\begin{NiceMatrix}
      z                 & \bloch{3-1}{\rppprec[f,g]} & z + H \big( (n)_1, (n)_2 \big)  \\
      n                 &                            & n                               \\
      \boldsymbol{0}    &                            & \boldsymbol{0}                  \\
    \end{NiceMatrix}\]
    \end{minipage}
...
This conclude our discussion of the proof of \lstinline{completeness}.
