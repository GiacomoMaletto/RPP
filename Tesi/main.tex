...
For example, the selection symbol $\mathsf{If}$ should be followed by three other $\RPP$ functions;
if we write $\mathsf{If}[\rppSu, \rppPr] \rppCo \rppNe$ we get a non-valid sentence.
...
\begin{lstlisting}
#check It Su ;; ((Id 1) f If Sw Pr Su) -- RPP
\end{lstlisting}
...
We're almost ready to define \lstinline{evaluate}.
What we don't know yet is how to represent and work with integer tuples, $\Z^n$.
...
\begin{lstlisting}
open list
#reduce [4, 5, 7, 2, 5] -- outputs "[4, 5, 7, 2, 5]"
\end{lstlisting}
...
#reduce 2 :: [4, 5, 7, 2, 5]   -- the same thing  
\end{lstlisting}
...
The function $\mathsf{evaluate}$ should take $\RPP$-sentences and return functions $\Z^n \to \Z^n$,
