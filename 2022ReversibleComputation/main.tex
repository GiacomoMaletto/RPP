...
an \lstinline|f| exists in \RPP that encodes $ F $, meaning that, for every $ \overline{x}\in \NN^n $,
$ \mbox{\lstinline|f(|}z,\overline{x},\boldsymbol{0}\mbox{\lstinline|) = (|}z+F(\overline{x}),\boldsymbol{0}\mbox{\lstinline|)|}$. The boldface $ \boldsymbol{0} $ is a suitable amount of $ 0 $, typically called \emph{ancille}, which are a temporary storage for intermediate computations of the encoding;
...
\begin{figure}
    \centering
        ...
        | Su : RPP         -- Successor
        | Pr : RPP         -- Predecessor
        | Sw : RPP         -- Transposition or Switch
...
\lstinline|Sw| is the transposition, or switch, that exchanges the first two elements of its argument.
