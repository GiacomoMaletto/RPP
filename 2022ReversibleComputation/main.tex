...
Section~\ref{section:The UPRF-completeness of RPP}, once recalled, and commented the main aspects of \lstinline|primerec|, illustrates the key step to port the original proof that \RPP is \PRF-complete to \LEAN.
...
\begin{itemize}
    ...
    Last, the termination of \lstinline|It| is simpler to check \LEAN, as compare to \lstinline|ItR|, \lstinline|for|.
%    Last, it is much easier to convince \LEAN that \lstinline|It| terminates, than it is for \lstinline|ItR| or \lstinline|for|.
\end{itemize}
...
\begin{remark}
The function \lstinline|cp| in Figure~\ref{sfig:cp} can replace \lstinline|mkpair| in Figure~\ref{sfig:encode prec 3rd step} as both isomorphically map $ \NN^2$ into $\NN$. Indeed, the original \PRF-completeness of \RPP relies on \lstinline|cp|. We favor \lstinline|mkpair| to take the most out of \MATHLIB. \qeda
\end{remark}
...
A more speculative direction, is to keep exploring the existence of programming schemes in \lstinline|RPP| able to generate functions, other than Cantor Pairing, etc., which we can see a discrete space-filling functions, whose behavior we can describe as steps along a path in some space.
