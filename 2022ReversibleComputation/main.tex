...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item we can take advantage of \lstinline|mkpair|, \lstinline|unpair|, and their properties in \MATHLIB; they implement an isomorphism $ \NN \simeq \NN^2 $ that we uses as a stack to encode any \lstinline|f:primrec| as a function in \lstinline|RPP|. Instead of \lstinline|mkpair|/\lstinline|unpair|, the original proof in \cite{DBLP:journals/tcs/PaoliniPR20} relies on Cantor Pairing \cite{Cantor1878,DBLP:journals/corr/Szudzik17} in \RPP.
%        We recall that Cantor Pairing is an isomorphism $ \mathbb{N}\times\mathbb{N} \simeq \mathbb{N} $. Inside the original proof of \PRF-completeness, Cantor Pairing works as a stack in order to correctly manage the computation of a recursive scheme of \UPRF by means of an element in \RPP. The \PRF-completeness that we develop in \LEAN does not rely on Cantor Pairing, but on an isomorphism $ \mathbb{N}\times\mathbb{N} \simeq \mathbb{N} $ in \MATHLIB \LEAN library.
    ...
    \item if we had no \lstinline|mkpair|/\lstinline|unpair| available, however, we would had to rely on some implementation of Cantor Pairing as in the original proof. We see as a further result the definition of a Cantor Pairing constructor \lstinline|cp|, and of a Cantor Pairing destructor \lstinline|cu| which we obtain as specific instance of a more general pattern which uniquely associates pairs in $ \mathbb{N}\times\mathbb{N}$ to a single element of $ \mathbb{N}$ by counting steps in specific paths.
\end{itemize}
...
Section~\ref{section:The UPRF-completeness of RPP} once recalled and commented the main aspects of \lstinline|primerec|, it illustrates the key step to port the original proof that \RPP is \PRF-complete to \LEAN.
...
If \lstinline|X.length >= f.arity|, i.e. \lstinline|X.length| supplies enough arguments, then \lstinline|f| operates the first elements of \lstinline|X| according to its arity. This justifies \textit{Remark}~\ref{remark:Weakening algorithms of RPP}. If \lstinline|X.length < f.arity|, i.e. \lstinline|X| has not enough elements less, \lstinline|f X| has an unspecified behavior. This might sounds odd, but it simplifies the proofs of must-have properties of \lstinline|RPP|.
...
In Figure~\ref{sfig:cp input}, \lstinline|cp_in| is the natural algorithm in \lstinline|RPP| to implement~\eqref{align: cp analitically}. As expected, the input pair $(x,y)$ is part of \lstinline|cp_in| output. The suffix ``\lstinline|in|'' in the name ``recalls'' exactly this aspect. In order to drop $(x,y)$ from the output, and obtain \lstinline|cp| as in Figure~\ref{sfig:cp}, we need \lstinline|cu_in⁻¹|, i.e. \lstinline|cu_in| which is new, as compared to \cite{DBLP:journals/tcs/PaoliniPR20}. The intuition behind \lstinline|cu_in| works as follows. Let us fix any point $ \rppcp(x, y) \in \NN $. We can realize that, starting from the origin, if we follow as many steps as the value $ \rppcp(x, y) $ in Figure~\ref{sfig:cantor as a path}, we stop exactly at $ (x,y) \in \NN^2 $. The function, expressed in standard functional notation, that, given the current point $ (x,y) $, identifies the next one to move to in the path of Figure~\ref{sfig:cantor as a path} is:
\begin{align*}
...
\begin{remark}
Driven by need to write a simple reversible function \lstinline|cu_in⁻¹| able to collect the ``garbage'' that \lstinline|cp_in|, we ended up with ignoring completely the analytical definition of $ \rppcu(n) $ in~\eqref{align: cu analitically}.
\qeda
\end{remark}
...
The scheme \lstinline|pair| builds the function that, taken a value \lstinline|n:ℕ|, gives the unique value in \lstinline|ℕ| that encodes the pair of values \lstinline|G n|, and \lstinline|G n|; everything we might pack up by means of \lstinline|pair|, we can unpack with \lstinline|left|, and \lstinline|right|.
...
\end{aligned}
\end{equation}
where is the built-in recursive scheme \lstinline|nat.rec| on \lstinline|ℕ|, and $ \gl a, b \gr $ denotes $ (\mbox{\lstinline|mkpair|}\ a\ b) $.
...
\newcommand{\bpiinv}{\bloch{3-1}{\mbox{\lstinline|H⁻¹|}}}
...
\scalebox{.9}
{$\begin{NiceMatrix}
 z  &\bid&\bid&\binc&\bid   &\bid& z + z'+ H[\mbox{\lstinline|F|},\mbox{\lstinline|G|}] (n)\\
 n  &\bsw&\bpi&     &\bpiinv&\bsw& n           \\
 z' &    &    &     &       &    & z'          \\
 \bz&    &    &     &       &    &\bz
 \end{NiceMatrix}$}
...
\noindent
Figure~\ref{sfig:prec[f,g]}, where we assume $ z, z'= 0 $, defines \lstinline|prec[f,g]:RPP| such that \lstinline|(encode (prec F G) prec[f,g]):Prop| holds, and \lstinline|H[f,g]| encodes
...
\end{NiceMatrix}$
    ...
    \caption{Build the argument $ R[\mbox{\lstinline|G|}](z,n) $ of \lstinline|g|.}
...
\begin{remark}
For example, \lstinline|cp| in Figure~\ref{sfig:cp} could replace \lstinline|mkpair| in Figure~\ref{sfig:encode prec 3rd step} because both are an isomorphism $ \NN \simeq \NN^2 $.
The proof of \PRF-completeness of \RPP in \cite{DBLP:journals/tcs/PaoliniPR20} relies on \lstinline|cp|. We favor \lstinline|mkpair| to better exploit its properties in \MATHLIB.
\qeda
\end{remark}
