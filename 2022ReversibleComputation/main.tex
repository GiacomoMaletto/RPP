...
The function \lstinline|ev| in Figure~\ref{fig:RPP-ev} interprets an element of \lstinline|RPP| as a function from a list of integers to a list of integers. Originally, in \cite{DBLP:journals/tcs/PaoliniPR20}, \RPP is a class of functions with type $ \ZZ^n \rightarrow \ZZ^n $. We use \lstinline|list ℤ| in place of tuples of \lstinline|ℤ| to exploit \MATHLIB, the rich \LEAN library of properties to save quite a large amount of formalization work.
...
\begin{itemize}
    ...
    \item The finite iterator \lstinline|It f (x::t): list ℤ| \emph{subsumes} the finite iterators \lstinline|ItR| in \RPP, and \lstinline|for| in \SRL, i.e. \lstinline|It| is more primitive but equally expressive.
    ...
    Last, the termination of \lstinline|It| \LEAN is simpler to check than for \lstinline|ItR| or \lstinline|for|.
\end{itemize}
...
In order to formally state what we mean for \lstinline|RPP| to be \PRF-complete we need to say \LEAN when, given \lstinline|F:ℕ → ℕ|, we can \emph{encode} it by means of some \lstinline|f:RPP|:
\begin{lstlisting}
