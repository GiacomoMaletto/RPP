...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        The pattern is obtained through the iteration of a function \lstinline|step(_)|, and becomes a reversible element in \lstinline|rpp| once fixed the parameter \lstinline|(_)| it depends on. Slightly different parameter instances generate reversible algorithms whose behavior we can certify in \LEAN. They are truncated Square Root, Quotient/Reminder of integer division, and Cantor Pairing \cite{Cantor1878,DBLP:journals/corr/Szudzik17}.
...
More precisely, suppose that $i\in\NN$, that $(x,y)\in\NN^2$ is a point in the $2$-dimensional grid $\NN^2$, that $\boldsymbol z\in\ZZ^n$ is additional data and, finally, that $f:\NN\times\ZZ^n\to\NN\times\ZZ^n$ is a function. We want the following behavior: starting from $(x,y)$, we take $i$ ``steps'' in $\NN^2$.
A \emph{step} is an update rule. It move position $(x,y) $ to $(x',y')$, written $(x,y)\mapsto(x',y')$, and data $\boldsymbol z$ to $\boldsymbol z'$, i.e. $\boldsymbol z\mapsto\boldsymbol z'$, as follows:
\begin{enumerate}
\item
\label{enum: downward steps}
if $y>0$, we pose $(x,y)\mapsto(x+1,y-1)$ and $\boldsymbol z\mapsto\boldsymbol z$;
\item
\label{enum: upward step}
if $y=0$, we pose $(x,0)\mapsto(0,y')$ and $\boldsymbol z\mapsto\boldsymbol z'$ where $(y',\boldsymbol z')=f(x,\boldsymbol z)$.
\end{enumerate}
In Figure~\ref{fig:path}, the downward arrows represent steps as described at point~\ref{enum: downward steps} here above, while the upward arrow represents the one defined at point \ref{enum: upward step}.

A seemingly straightforward way to implement this behavior in \lstinline|rpp| would be the following: move the argument $y$ to the head; use the conditional \lstinline|If| to perform either a diagonal movement or the jump from the $x$-axis to the $y$-axis, depending on the sign of $y$. Unfortunately this does not work, because \lstinline|rpp| (and more deeply, the constraints of reversibility) prevent a variable to be in both the condition of \lstinline|If| and among the affected variables. The current implementation of \lstinline|step| is illustrated in Figure~\ref{fig:function scheme step}. It avoids the issue by using an additional variable which is set to $1$ (respectively $0$), depending on whether $y$ is $>0$, (respectively $=0$), eventually setting it back to $0$ in either cases based on the affected variables.
...
On top of the functions in Figures~\ref{fig:standard functions}, and~\ref{fig:function scheme step} we build Cantor Pairing/Un-pairing, Quotient/Reminder of integer division, and truncated Square Root which correspond to visiting $ \NN^2 $ as in Figures~\ref{sfig:cantor as a path}, \ref{sfig:Quot./Rem.}, and~\ref{sfig:Square root}, respectively. The pairing function $\rppmkpair$, which behaves as in Figure~\ref{sfig:mkpair}, and which is an alternative to Cantor Pairing/Unpairing, has a more complex definition; it will be a necessary ingredient of our main proof.
...
\boldsymbol{z'}\\
\end{NiceMatrix}$}
\caption{Function \lstinline|step f|; detailed behavior with $y=0$, assuming $x\ge0$ and $(y',\boldsymbol{z'})=f(x,\boldsymbol{z})$.}
\end{subfigure}
...
An analytic definition of $\rppmkpair:\NN^2\to\NN$ is:
\[
\rppmkpair(x,y)=\begin{cases}y^2+x&\text{if }x<y\\x^2+x+y&\text{otherwise}\end{cases}
\]
\noindent
whose inverse $\rppunpair:=\rppmkpair^{-1}:\NN\to\NN^2$ is:
\[
\rppunpair(n)=\begin{cases}(n-\floor{\sqrt n}^2,\floor{\sqrt n})&\text{if }n-\floor{\sqrt n}^2<\floor{\sqrt n}\\(\floor{\sqrt n},n-\floor{\sqrt n}^2-\floor{\sqrt n})&\text{otherwise}
\enspace .
\end{cases}
\]
\noindent
...
by \LEAN, we always work up to automorphisms of \lstinline|ℕ|
